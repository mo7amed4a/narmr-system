  ...
  return (
    <Card className='flex flex-col md:flex-row justify-between shadow-none border-none bg-[#F6F6F6]'>
        <CardHeader className='flex-1 flex flex-col md:flex-row gap-4 p-2'>
           <img src={note.attached_file} className='md:size-14 h-40  p-1'/>
           <div>
            <a href="#" className='text-gray-600 text-sm'>{note.file_name}</a>
            <div className='flex gap-3'>
              <p className='text-gray-400 text-xs'>{note.note_text}</p>
        ...
        <CardContent className='flex items-center'>
         ...
         <ModalEditNote note={note} setRefresh={setRefresh} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </Card>
  )
}
