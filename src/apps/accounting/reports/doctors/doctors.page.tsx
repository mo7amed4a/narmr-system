...
export default function DoctorsAccountingPage() {
  const [value, setValue] = useState("all");
  const [data, setData] = useState(transactions)

  useEffect(() => {
    if (value === "complete") {
      const d = transactions.filter(item => item.status === true)
      setData(d)
    }
    else if (value === "canceled") {
      const d = transactions.filter(item => item.status === false)
      setData(d)
    }
    else {
        ...
        <Card className=" border-none shadow-none">
          <CardContent className="pt-4 space-y-4">
            ...
            <div className="flex items-center flex-nowrap gap-x-2">
              ...
              <Label htmlFor="all" className="text-nowrap">الكل</Label>
            </div>
            <div className="flex items-center flex-nowrap gap-x-2">
              <RadioGroupItem value="complete" id="complete" />
              <Label htmlFor="complete" className="text-nowrap">الحجوزات المكتملة</Label>
            </div>
            <div className="flex items-center flex-nowrap gap-x-2">
              <RadioGroupItem value="canceled" id="canceled" />
              <Label htmlFor="canceled" className="text-nowrap">الحجوزات الملغاة</Label>
            </div>
              </RadioGroup>
